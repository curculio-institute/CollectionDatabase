...
class ExternalIdentifier(Base, TimestampMixin):
    ...
    iNaturalist observation URL). For a **biological_association** it denotes the *other
    party* — the non-collection-object side — as an external resource (e.g. the host
    plant's iNaturalist observation). It is an **optional addition**: the association's
    object arc (taxon / specimen) in biological_association is unchanged.

    Exclusive-arc: exactly one of (collection_object_id, biological_association_id) is set
    — the project's FK-safe alternative to a polymorphic association.
    """
